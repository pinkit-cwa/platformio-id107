...
class BleDfuControllerLegacy(NrfBleDfuController):
    # Class constants
    JJUUID_CONTROL_POINT   = "00001531-1212-efde-1523-785feabcd123"
    UUID_CONTROL_POINT   = "00000af6-0000-1000-8000-00805f9b34fb"
    #UUID_CONTROL_POINT   = "00000af0-0000-1000-8000-00805f9b34fb"
    ...
    def start(self, verbose=True):
        (_, self.ctrlpt_handle, self.ctrlpt_cccd_handle) = self._get_handles(self.JJUUID_CONTROL_POINT)
    ...
    def switch_to_dfu_mode(self):
        ...
        print "write something to bluetoothservice to switch to DFU"
        #cmd = 'char-write-req 0x000e 0x01' 
        #cmd = 'char-write-req 0x%02x %02x' % (bl_cccd_handle, 1)
        cmd = 'char-write-req 0x%02x %02x' % (bl_value_handle, 1)
        #if verbose: print cmd
        self.ble_conn.sendline(cmd)
        time.sleep(5)
        ret = self.scan_and_connect()
        if verbose: print "Connected 1" + str(ret)
        #self.ble_conn.sendline(cmd)
        #time.sleep(5)
        #ret = self.scan_and_connect()
        #if verbose: print "Connected 2" + str(ret)
        #self.ble_conn.sendline(cmd)
        #time.sleep(5)
        #ret = self.scan_and_connect()
        #if verbose: print "Connected 3" + str(ret)
        #self.ble_conn.sendline(cmd)
        #time.sleep(5)
        #ret = self.scan_and_connect()
        #if verbose: print "Connected 4" + str(ret)
